...
int main( void )
{
	...
	window = glfwCreateWindow( 1024, 768, "Lab5 wtf", NULL, NULL);
	if( window == NULL ){
	...
	// shader for the "scene" (which has ambient occlusion texture in the ambient channel that is multiplied by diffuse colour texture)
	...
	Shader BlendShader("StandardShadingAOSpecVertexBlend.vertexshader", "StandardShadingAOSpecVertexBlend.fragmentshader");
	check_gl_error();
	...
	Model sceneModel("../3dcontent/models/plane/scene.fbx");
	check_gl_error();

	// the vertex blend mesh mesh, in this case a plane
	Model blendModel("../3dcontent/models/plane/planeX.dae",true);
	...
	do{
		...
		sceneModel.Render(View, Projection, StandardShaderWithAO);
